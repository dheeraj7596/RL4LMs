...
class IMDB(TextGenPool):
    ...
    """
    @classmethod
    def prepare(cls, split: str, seed: int, prompt_suffix: str = "", prompt_prefix: str = ""):
        ...
        if split in ["train", "val"]:
            ...
            if split == "train":
                dataset_split = dataset_split[:train_index]
            else:
                dataset_split = dataset_split[train_index: train_index + 1000]
        ...
        for ix, text in enumerate(dataset_split["text"]):
            ...
            prompt_text = prompt_prefix + " ".join(prompt_text[:int(len(prompt_text) * 0.5)]) + prompt_suffix
